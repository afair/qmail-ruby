  ...
  module Qmail
    ...
    class SMTP
      ...
      def self.sendmail(mail_tools_message)
        MailTools::SMTP.new(mail_tools_message).sendmail
      end
      ...
      # Sends email directly via mail_tools-remote. It does not store in the queue, It will halt the process
      # and wait for the network event to complete. If multiple recipients are passed, it will run
      # mail_tools-remote delivery for each at a time to honor VERP return paths.
      def sendmail
        cmdfile = "#{@qmsg.options[:mail_tools_root]}/bin/qmail-remote"
        ...
        @qmsg.recipients.each do |recip|
          ...
          #@options[:logger].info("RubyMailTools Remote #{recip} exited:#{@success} responded:#{@response}")
        end
      ...
      # mail_tools-queue does not work with this as it reads from both pipes.
      def spawn_command(command, &block)
        ...
        unless @child # 
          ...
          Dir.chdir(@options[:mail_tools_root]) unless @options[:nochdir]
          exec(command)
